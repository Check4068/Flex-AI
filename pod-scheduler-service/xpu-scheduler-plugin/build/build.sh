...
function build() {
  ...
  for name in controller-manager scheduler webhookmanager; do
    CGO_CFLAGS="-fstack-protector-strong -D_FORTIFY_SOURCE=2 -O2 -fPIC -ftrapv" \
    CGO_CPPLAGS="-fstack-protector-strong -D_FORTIFY_SOURCE=2 -O2 -fPIC -ftrapv" \
  ...
  -X volcano.sh/volcano/pkg/plugins/xpu-scheduler-plugin.PluginName=${PLUGIN_NAME}" \
  -o "${PLUGIN_NAME}".so "${GOPATH}"/src/volcano.sh/volcano/pkg/plugins/xpu-scheduler-plugin/
